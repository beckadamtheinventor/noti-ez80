...
_nomatch:
	...
	call	noti._CpHLDE			; have we reached the end of the table?
	push	af
	ex	de,hl
...
_notextracted:
	...
	call	noti._Mov8b			; copy the string. it shouldn't be bigger than this
	xor	a
	ld	(de),a
	inc	de
...
_findbinary:
	call	noti._LoadLibraryOP1 ;returns de = start of library, hl = size of library
...
_goodversion:
	...
	call	noti._CpHLDE			; check and see if they match -- if so, this library is going to remain in the archive
...
_relocateabsolutesloop:
	ld	de,(endrelocationtbl)
	call	noti._CpHLDE			; have we reached the end of the relocation table
...
_nosetstart:
	...
	call	noti._CpHLDE			; make sure we are done parsing the dependency queue
