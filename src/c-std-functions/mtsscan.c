...
#else
static unsigned char pointer(char _PTR_ bptr,va_list argp,int len,unsigned char fields,struct fmt_type fmt_str)
#endif
{
  unsigned char i;
  char ch;
...
#else
static unsigned char string(char _PTR_ bptr,va_list argp,int len,unsigned char fields,struct fmt_type fmt_str)
#endif
{
  unsigned char i;
  char ch;
...
#else
static unsigned char character(char _PTR_ bptr,va_list argp,int len,unsigned char fields,struct fmt_type fmt_str)
#endif
{
  unsigned char i;
  char ch;
...
#else
static unsigned char fpoint(char _PTR_ bptr,va_list argp,int len,unsigned char fields,struct fmt_type fmt_str)
#endif
{
  unsigned char i;
  char ch;
...
#else
static unsigned char scalar(char _PTR_ bptr,va_list argp,int len,unsigned char fields,struct fmt_type fmt_str,int radix)
#endif
{
  unsigned char i;
  char ch;
...
#else
static unsigned char set(char _PTR_ bptr,va_list argp,int len,struct fmt_type fmt_str)
#endif
{
  char _PTR_ p2;
  char neg,ch;
  ...
  while ( ok && prev_ch!=EOF && *fmt) {
    ...
    if (fmt_str.status == FMT_ERR)
	  ...
	  _sclr:
				...
				ok = scalar(radix);
#else
				ok = scalar(&src,&argp,&len,&fields,&fmt_str,radix);
#endif
	  ...
	  case 'g':
                ...
                ok = fpoint();
#else
				ok = fpoint(&src,&argp,&len,&fields,&fmt_str);
#endif
	  ...
	  case 'c':
		...
		ok = character();
#else
		ok = character(&src,&argp,&len,&fields,&fmt_str);
#endif
		break;
	  case 's':
#ifndef _MULTI_THREAD
		ok = string();
#else
		ok = string(&src,&argp,&len,&fields,&fmt_str);
#endif
		break;
	  case 'p':
#ifndef _MULTI_THREAD
		ok = pointer();
#else
		ok = pointer(&src,&argp,&len,&fields,&fmt_str);
#endif
		break;
	  case 'n':
		if (!(fmt_str.flags & FMT_FLAG_IGNORE)) {
