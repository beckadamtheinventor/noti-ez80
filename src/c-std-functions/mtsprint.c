...
{
#ifdef _MULTI_THREAD
#define __print_putch(ch) *dest++=ch
#define __print_send() __print_len += __mt_sprint_send(&dest,'\0',__print_buff,&__print_fmt)
#define __print_sendstring(s) __print_len+=__mt_sprint_sendstring(&dest,s,&__print_fmt)
#define _u_ltoa(l) __print_len+=__mt_sprint_send(&dest,_mt_ltoa(l,__print_buff,&__print_fmt),__print_buff,&__print_fmt)
#define _u_ultoa(l) _mt_ultoa(l,__print_buff,&__print_fmt);__print_len+=__mt_sprint_send(&dest,'\0',__print_buff,&__print_fmt)
#define _u_dtof(d) __print_len+=__mt_fsprint_send(&dest,_mt_dtof(d,__print_buff,&__print_fmt),__print_buff,&__print_fmt)
#define _u_dtog(d) __print_len+=__mt_fsprint_send(&dest,_mt_dtog(d,__print_buff,&__print_fmt),__print_buff,&__print_fmt)
#endif
  ...
  while (*fmt) {
    ...
    if (*fmt) {         /* we found a % in format string */
      ...
      if (__print_fmt.status == (unsigned char)FMT_ERR) {
		...
		break;
          case 'p':
                _u_ultoa((long)va_arg(argp,unsigned short));
		break;
          case 'n':
                sp = va_arg(argp,char _PTR_);
                if (__print_fmt.size == (char)'l' || __print_fmt.size == (char)'L')
